...
class DeviceListViewModel
    ...
    constructor(
        ...
        data class DeviceItemUiState(
            ...
            val address: String,
            val name: String,
            // The name to show (whitelist label if known, else device name)
                                ...
                                try {
                                    locationRepository.getLocationsForDeviceOnce(device.id).map { location ->
                                        LocationPreview(
                                            ...
                                            // Location entity doesn't have a name
                                            name = null,
                    ...
                    val deviceItems =
                        devices.map { device ->
                            ...
                            device.toUiState().copy(
                                ...
                                displayName =
                                    if (isWhitelisted && !whitelistEntry?.label.isNullOrBlank()) {
         ...
         */
        fun tagDeviceAsKnown(
            deviceId: Long,
            label: String,
            category: String = WhitelistRepository.Category.TRUSTED,
        ) {
            viewModelScope.launch {
                try {
        ...
        private fun ScannedDevice.toUiState(): DeviceItemUiState {
            ...
            val rawManufName =
                manufacturerName ?: manufacturerId?.let { id ->
            ...
            return DeviceItemUiState(
                ...
                address = address,
                name = deviceName,
                // Will be overridden if whitelisted
