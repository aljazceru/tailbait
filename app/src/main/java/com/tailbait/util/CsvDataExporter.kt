...
class CsvDataExporter
    ...
    constructor(
        ...
        suspend fun exportAllData(): ExportResult =
            withContext(Dispatchers.IO) {
                try {
                    ...
                    Timber.i(
                        "Data loaded: ${devices.size} devices, ${locations.size} locations, " +
                            "${deviceLocationRecords.size} device-location records, " +
                            "${userPaths.size} user path points, ${alerts.size} alerts, " +
                    ...
                    val zipFile =
                        File(
                            ...
                            "bletracker_export_${SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(Date())}.zip",
                        )
         ...
         */
        private fun exportDeviceLocationRecords(
            ...
            FileWriter(file).use { writer ->
                // CSV Header
                writer.appendLine(
                    "id,device_id,location_id,rssi,timestamp,location_changed,distance_from_last,scan_trigger_type",
                )
        ...
        private fun exportAlerts(
            ...
            FileWriter(file).use { writer ->
                // CSV Header
                writer.appendLine(
                    "id,alert_level,title,message,timestamp,device_addresses," +
                        "location_ids,threat_score,detection_details,is_dismissed,dismissed_at,created_at",
                )
        ...
        private fun createReadmeFile(
            ...
            FileWriter(file).use { writer ->
                ...
                writer.appendLine(
                    "App Version: ${com.tailbait.BuildConfig.VERSION_NAME} (${com.tailbait.BuildConfig.VERSION_CODE})",
                )
                ...
                writer.appendLine(
                    "- `apple_continuity_type`: Continuity message type (7=AirPods, 16=NearbyInfo, 18=FindMy)",
                )
                writer.appendLine("")
                ...
                writer.appendLine(
                    "- Join `user_paths.location_id` → `locations.id` to get GPS coordinates for movement history",
                )
                writer.appendLine(
                    "- For alerts: Parse `device_addresses` and `location_ids` JSON arrays to relate to devices and locations",
                )
                writer.appendLine("")
                ...
                writer.appendLine("")
                writer.appendLine(
                    "This export contains location data and device information. " +
                        "Handle with appropriate privacy considerations and " +
                        "only use for legitimate security analysis purposes.",
                )
            }
        }
