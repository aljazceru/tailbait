...
class ShadowAnalyzerTest {
    ...
    @Test
    fun `all-ones counts produce high persistence score`() =
        runTest {
            ...
            coEvery { deviceRepository.getShadowLocationDeviceCounts(shadowKey) } returns
                listOf(
    ...
    @Test
    fun `varying counts produce low persistence score`() =
        runTest {
    ...
    @Test
    fun `whitelisted devices are excluded from results`() =
        runTest {
    ...
    @Test
    fun `no shadow keys returns empty results`() =
        runTest {
            val locations = (1L..3L).map { location(it) }
            every { locationRepository.getAllLocations() } returns flowOf(locations)
    ...
    @Test
    fun `rotation evidence boosts combined score`() =
        runTest {
            val locations = (1L..4L).map { location(it) }
            every { locationRepository.getAllLocations() } returns flowOf(locations)
            coEvery { deviceRepository.getSuspiciousShadowKeys(any()) } returns listOf(shadowKey)
    ...
    @Test
    fun `representative device is most recently seen`() =
        runTest {
            val locations = (1L..3L).map { location(it) }
            every { locationRepository.getAllLocations() } returns flowOf(locations)
            coEvery { deviceRepository.getSuspiciousShadowKeys(any()) } returns listOf(shadowKey)
            ...
            if (results.isNotEmpty()) {
                assertEquals(
                    ...
                    3L,
                    results.first().representativeDevice.id,
                )
            }
        }
}
