...
class DeviceListViewModelTest {
    ...
    fun `state loads with devices correctly`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
    ...
    @Test
    fun `devices are sorted by last seen descending by default`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `updateSortOption sorts by name ascending`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `updateSortOption sorts by name descending`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `updateSortOption sorts by detection count descending`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `updateSortOption sorts by detection count ascending`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `updateSortOption sorts by first seen descending`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `updateSearchQuery filters by device name`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `updateSearchQuery filters by device address`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `updateSearchQuery filters by device type`() =
        runTest {
    ...
    @Test
    fun `updateSearchQuery with empty string shows all devices`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `clearSearch clears search query`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
    ...
    @Test
    fun `refreshDevices sets isRefreshing temporarily`() =
        runTest {
            viewModel = createViewModel()
            advanceUntilIdle()
    ...
    @Test
    fun `getSortOptionDisplayName returns correct names`() =
        runTest {
            viewModel = createViewModel()
            advanceUntilIdle()
            ...
            assertEquals(
                "Last Seen (Newest)",
                viewModel.getSortOptionDisplayName(DeviceListViewModel.SortOption.LAST_SEEN_DESC),
            )
            assertEquals(
                "Last Seen (Oldest)",
                viewModel.getSortOptionDisplayName(DeviceListViewModel.SortOption.LAST_SEEN_ASC),
            )
            assertEquals(
                "First Seen (Newest)",
                viewModel.getSortOptionDisplayName(DeviceListViewModel.SortOption.FIRST_SEEN_DESC),
            )
            assertEquals(
                "First Seen (Oldest)",
                viewModel.getSortOptionDisplayName(DeviceListViewModel.SortOption.FIRST_SEEN_ASC),
            )
            assertEquals(
                "Detection Count (High-Low)",
                viewModel.getSortOptionDisplayName(DeviceListViewModel.SortOption.DETECTION_COUNT_DESC),
            )
            assertEquals(
                "Detection Count (Low-High)",
                viewModel.getSortOptionDisplayName(DeviceListViewModel.SortOption.DETECTION_COUNT_ASC),
            )
        }

    @Test
    fun `devices with null names are displayed as Unknown Device`() =
        runTest {
            val deviceWithoutName =
                ScannedDevice(
                    id = 4L,
                    address = "FF:FF:FF:FF:FF:FF",
                    name = null,
    ...
    @Test
    fun `search and sort work together correctly`() =
        runTest {
            coEvery { deviceRepository.getAllDevices() } returns
                flowOf(
                    listOf(testDevice1, testDevice2, testDevice3),
